...
class StratumSession(RPCSession):
    ...
    async def handle_authorize(self, username: str, password: str):
        ...
        if base58.b58decode_check(address)[0] != (42 if self._testnet else 38):
            ...
            try:
                ...
                if new_block or new_witness or state.timestamp + 60 < ts:
                    ...
                    state.coinbase_tx = (int(1).to_bytes(4, 'little') + \
                                    ...
                                    b'\x01' + coinbase_txin + \
                                    b'\x03' + \
                                        int(coinbase_sats_int*0.85).to_bytes(8, 'little') + op_push(len(vout_to_miner)) + vout_to_miner + \
                                        int(coinbase_sats_int*0.15).to_bytes(8, 'little') + op_push(len(vout_to_devfund)) + vout_to_devfund + \
